...
export default function EventListItem({ event }: EventListItemProps) {
  ...
  const eventListItemIcon = (event: CalendarEvent) => {
    ...
    if (eventActive(event.start.dateTime, event.end.dateTime)) return { source: Icon.CircleProgress, tintColor: Color.Green  }
  ...
  return (
    <List.Item
      ...
      accessories={[{ text: `${parseTime("start", startDate)} - ${parseTime("end", endDate)}` || "unknown" }, { icon: getFavicon(meetingLinkIcon(event)) }]}
      actions={
        <ActionPanel>
